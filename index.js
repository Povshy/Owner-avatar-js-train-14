...
function nextNewYear() {
  ...
  let ny = new Date(todayYear + 1, 0, 1);
...
function addDays(date, days) {
  ...
  const todays = inputDate.getDate();
  const result = inputDate.setDate(todays + days);
...
function getDaysInMonth(date) {
  ...
  // let nextM = new Date(year, month + 1, 1);
...
function getAge(birthDate) {
  ...
  let years = inputDate.getFullYear() - birthDate.getFullYear();
  ...
  let BirthDate = birthDate.toISOString();
...
function compareDates(date1, date2) {
  ...
  let Date1 = date1.toISOString();
  let Date2 = date2.toISOString();
...
function getDaysDifference(startDate, endDate) {
  ...
  let StartDate = startDate.getTime();
  let EndDate = endDate.getTime();
  let Diff = EndDate - StartDate;
  // Перетворення різниці в мілісекундах у дні поділивши мілісекунди на (1000 * 60 * 60 * 24).
  let diff = Diff / (1000 * 60 * 60 * 24);
